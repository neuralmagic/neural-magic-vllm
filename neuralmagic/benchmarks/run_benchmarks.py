...
def run_bench(server_cmd: str, bench_cmd : list[str]) -> None:
    try:
        ...
        if (not is_server_running(BENCH_SERVER_HOST, BENCH_SERVER_PORT)):
            raise ValueError(f"Aborting bench run with : server-cmd {server_cmd} , bench-cmd {bench_cmd}. Reason: Cannot start Server") 
        # run bench
...
def run_benchmark_serving_script(config: NamedTuple, output_directory: Path) -> None:
    ...
    for download_cmd in config.dataset_download_cmds:
        download_cmd_as_list = list(filter(lambda x: len(x) != 0, download_cmd.split(' ')))
    ...
    for model in config.models:
        ...
        for script_args in script_args_to_cla(script_args_kv):
            bench_cmd = ["python3", f"{script_path}"] + script_args + ["--save-directory",  f"{output_directory}"] + \
                                                                ["--model", f"{model}"] + \
                                                                ["--tokenizer", f"{model}"] + \
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Runs benchmark-scripts as a subprocess")
    parser.add_argument("-i", "--input-config-file", required=True, type=str, help="Path to the input config file describing the benhmarks to run")
    parser.add_argument("-o", "--output-directory", required=True, type=str, help="Path to a directory that is the output store")
