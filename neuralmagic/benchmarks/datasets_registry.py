...
def make_dataset_triples(
    ...
    for prompt, completion in zip(prompts, completions):
        ...
        if (prompt_len < 4 or output_len < 4 or 
            prompt_len + output_len > dataset_args.max_len):  
            continue
...
def get_ultrachat(tokenizer: PreTrainedTokenizerBase, dataset_args: DatasetArgs) -> DatasetTriple:
    # Load dataset.
    ds = load_dataset(
        "HuggingFaceH4/ultrachat_200k",
        split="train_sft[:10%]"
    ).shuffle(seed=dataset_args.seed).select(range(dataset_args.num_samples))
...
def get_dataset(
    ...
    if name not in _DATASET_REGISTRY:
        raise ValueError(f"{name} not found in dataset registry: {_DATASET_REGISTRY.keys()}")
    else:
        return _DATASET_REGISTRY[name](tokenizer=tokenizer, dataset_args=dataset_args)
