...
from ...modeling_outputs import BaseModelOutput # also in whisper modeling source
...
class WhisperGenerationMixin:
    ...
    def _maybe_warn_unused_inputs(
        ...
        if condition_on_prev_tokens is not None:
            print(warning_prefix.format(f"condition_on_prev_tokens is set to {condition_on_prev_tokens}"))

        if compression_ratio_threshold is not None:
            print(warning_prefix.format(f"compression_ratio_threshold is set to {compression_ratio_threshold}"))

        if logprob_threshold is not None:
            print(warning_prefix.format(f"logprob_threshold is set to {logprob_threshold}"))

        if no_speech_threshold is not None:
            print(warning_prefix.format(f"no_speech_threshold is set to {no_speech_threshold}"))
    ...
    def _set_return_timestamps(return_timestamps, is_shortform, generation_config):
        if not is_shortform:
            ...
            print("Setting `return_timestamps=True` for long-form generation.")
    ...
    def _set_num_frames(return_token_timestamps, generation_config, kwargs):
        if return_token_timestamps:
            if getattr(generation_config, "task", None) == "translate":
                printing("Token-level timestamps may not be reliable for task 'translate'.")
