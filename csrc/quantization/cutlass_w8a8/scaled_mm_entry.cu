...
void cutlass_scaled_mm_sm80_transpose(torch::Tensor& c, torch::Tensor const& a,
                            torch::Tensor const& b,
                            torch::Tensor const& a_scales,
                            torch::Tensor const& b_scales);
...
void cutlass_scaled_mm_transpose(torch::Tensor& c, torch::Tensor const& a,
  ...
  if (version_num >= 90) {
    ...
    assert (false && "nyi");
#else
    cutlass_scaled_mm_sm80_transpose(c, a, b, a_scales, b_scales);
#endif
