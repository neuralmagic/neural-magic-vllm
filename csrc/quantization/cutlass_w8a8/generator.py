...
class Cutlass2xGenerator(Generator):
    ...
    OPS_FILE= SCRIPT_DIR / "autogen_cutlass_ops.h"
    OPS_MACRO = "AUTOGEN_CUTLASS_DEFS"
    ...
    @staticmethod
    def generate_2x_file(arg: Cutlass2xArgs):
        ...
        code = ""
        fn_name = Cutlass2xGenerator.generate_name(arg)
        fn_decl = fn_decl_template.render(_name = fn_name)
        code += fn_defn_template.render(
                _name = fn_name,
                _tile_shape = get_as_cutlass_gemm_shape(arg.tile_shape),
                _warp_shape = get_as_cutlass_gemm_shape(arg.warp_shape),
                _instruction_shape = get_as_cutlass_gemm_shape(arg.instruction_shape),
