...
inline void cutlass_scaled_mm_with_epilogue_fp8(
...
template<typename Arch,
        ...
        int32_t MainLoopStages,
        template <typename, typename> typename Epilogue>
inline void cutlass_scaled_mm_with_epilogue_i8(
                  torch::Tensor& out, torch::Tensor const& a,
                  torch::Tensor const& b,
                  torch::Tensor const& a_scales,
                  torch::Tensor const& b_scales) {
...
template<typename Arch,
        ...
        GemmUniversalMode Mode,
        int32_t MainLoopStages>
inline void cutlass_scaled_mm_i8(torch::Tensor& out, torch::Tensor const& a,
                       torch::Tensor const& b,
                       torch::Tensor const& a_scales,
                       torch::Tensor const& b_scales) {
  TORCH_CHECK(a.dtype() == torch::kInt8);
  ...
  if (use_trivial_epilogue) {
      return cutlass_scaled_mm_with_epilogue_i8<Arch,
          TileShape, WarpShape, InstructionShape,
...
template<typename Arch,
        ...
        GemmUniversalMode Mode,
        int32_t MainLoopStages>
inline void cutlass_scaled_mm_fp8(torch::Tensor& out, torch::Tensor const& a,
                       torch::Tensor const& b,
                       torch::Tensor const& a_scales,
                       torch::Tensor const& b_scales) {
  ...
  bool const use_trivial_epilogue = a_scales.numel() == 0 && b_scales.numel() == 0;
  if (use_trivial_epilogue) {
      return cutlass_scaled_mm_with_epilogue_fp8<Arch,
          TileShape, WarpShape, InstructionShape,
          ThreadBlockSwizzle, Mode, MainLoopStages, TrivialEpilogue>(out, a, b, a_scales, b_scales);
  } else {
      return cutlass_scaled_mm_with_epilogue_fp8<Arch,
