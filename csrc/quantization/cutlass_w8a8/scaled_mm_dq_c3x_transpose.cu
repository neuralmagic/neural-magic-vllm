...
struct cutlass_3x_gemm_transpose {
  ...
  using ScaleB = cutlass::epilogue::fusion::Sm90ColOrScalarBroadcast<
      0 /*Stages*/, typename EpilogueDescriptor::TileShape, float,
      Stride<Int<1>, Int<0>, Int<0>>>;

  using ScaleADescriptor =
      cutlass::epilogue::collective::detail::RowBroadcastDescriptor<
          EpilogueDescriptor, float>;

  using ScaleA = cutlass::epilogue::fusion::Sm90RowOrScalarBroadcast<
      ScaleADescriptor::Stages, typename EpilogueDescriptor::TileShape,
      typename ScaleADescriptor::Element, Stride<Int<0>, Int<1>, Int<0>>>;
  ...
  using CollectiveEpilogue =
      typename cutlass::epilogue::collective::CollectiveBuilder<
          ...
          ElementAcc, float, ElementC, StrideC, 1, ElementD, StrideD, 1,
...
void cutlass_scaled_mm_dq_dispatcher_transpose(torch::Tensor& out_, torch::Tensor const& a_,
  ...
  auto a = b_.transpose(0, 1); // a is now column-major
  auto b = a_.transpose(0, 1); // b is now row-major
  auto out = out_.transpose(0, 1); // out is now column-major
...
struct sm90_fp8_config {
  ...
  using Cutlass3xGemm =
      cutlass_3x_gemm_transpose<InType, OutType, TileShape, ClusterShape, KernelSchedule,
                      EpilogueSchedule>;
};
...
template <typename InType, typename OutType>
void cutlass_scaled_mm_dq_sm90_fp8_dispatch_transpose(torch::Tensor& out,
  ...
  if (mp2 <= 64) {
    // m in [1, 64]
    return cutlass_scaled_mm_dq_dispatcher_transpose<Cutlass3xGemmM64>(
        out, a, b, a_scales, b_scales);
  } else if (mp2 <= 128) {
    // m in (64, 128]
    return cutlass_scaled_mm_dq_dispatcher_transpose<Cutlass3xGemmM128>(
        out, a, b, a_scales, b_scales);
  } else {
    // m in (128, inf)
    return cutlass_scaled_mm_dq_dispatcher_transpose<Cutlass3xGemmDefault>(
        out, a, b, a_scales, b_scales);
  }
}

void cutlass_scaled_mm_dq_sm90_transpose(torch::Tensor& out, torch::Tensor const& a,
                               torch::Tensor const& b,
                               torch::Tensor const& a_scales,
                               torch::Tensor const& b_scales) {
  TORCH_CHECK(a_scales.dtype() == torch::kFloat32);
  ...
  if (a.dtype() == torch::kInt8) {
    ...
    if (out.dtype() == torch::kBFloat16) {
      return cutlass_scaled_mm_dq_dispatcher_transpose<
          cutlass_3x_gemm_transpose<int8_t, cutlass::bfloat16_t, TileShape, ClusterShape,
      ...
      return cutlass_scaled_mm_dq_dispatcher_transpose<
          cutlass_3x_gemm_transpose<int8_t, cutlass::half_t, TileShape, ClusterShape,
                          KernelSchedule, EpilogueSchedule>>(
          out, a, b, a_scales, b_scales);
    }
  } else {
    ...
    if (out.dtype() == torch::kBFloat16) {
      return cutlass_scaled_mm_dq_sm90_fp8_dispatch_transpose<cutlass::float_e4m3_t,
                                                    cutlass::bfloat16_t>(
          out, a, b, a_scales, b_scales);
    } else {
      TORCH_CHECK(out.dtype() == torch::kFloat16);
