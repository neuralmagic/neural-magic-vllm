...
#include "cuda/marlinv2/marlinv2_mm_kernel.cuh"
#include "cuda/utilities/cute_utils.cuh"
#include "cuda/utilities/torch_utils.cuh"

namespace marlinv2 {

template <typename TileShapeMNL, typename ElementB, typename BInTensor, typename BTiledOutTensor>
static __global__ void prepack_B_kernel(BInTensor B_in, BTiledOutTensor B_tiled_out) {

  auto tB_in  = local_tile(B_in, TileShapeMNL{}, make_coord(blockIdx.x, blockIdx.y, blockIdx.z));
  auto tB_out = B_tiled_out(make_coord(_, _), make_coord(blockIdx.x, blockIdx.y), blockIdx.z);

  auto tiled_copy =
      make_tiled_copy(Copy_Atom<DefaultCopy, ElementB>{}, Layout<Shape<_4, _32>, Stride<_32, _1>>{},
                      Layout<Shape<_1, _2>>{});
...
static void prepack_B(cudaStream_t stream, typename PrepackedLayout::ElementB const* B_in_ptr,
                      InLayout B_layout, typename PrepackedLayout::ElementB* B_out_ptr) {
  using TileShapeMNL = decltype(append(typename PrepackedLayout::PPBlockShape_MK{}, _1{}));

  auto MKL_to_TVbMbK    = PrepackedLayout::pp_MKL_to_TVbMbK(shape(B_layout));
  auto TVbMbK_to_offset = PrepackedLayout::pp_TVbMbKL_to_offset(shape(B_layout));
  auto MKL_to_offset    = coalesce(composition(TVbMbK_to_offset, MKL_to_TVbMbK));

  auto MKbMbKL_to_offset = PrepackedLayout::pp_MKbMbKL_to_offset(shape(B_layout));
