...
struct PrepackedLayoutTemplate {
  ...
  // The N here doesnt actually impact the shape of the stored tile directly but may impact
  // the op selected by rs_op_selector
  using PPBlockShape_MNK =
      decltype(make_shape(size<0>(PPBlockShape_MK{}), _128{}, size<1>(PPBlockShape_MK{})));

  static constexpr cute::GMMA::Major GmmaMajorB = gmma_rs_tag_to_major_B<LayoutB>();
  using AtomLayoutMNK                           = cute::conditional_t<
      cute::is_same_v<KernelSchedule, KernelTmaWarpSpecializedCooperativeMixedInput>,
      Layout<Shape<_2, _1, _1>>, Layout<Shape<_1, _1, _1>>>;

  using TiledMma = decltype(cute::make_tiled_mma(
      cute::GMMA::rs_op_selector<ElementMma, ElementMma, ElementAccumulator, PPBlockShape_MNK,
  ...
  CUTE_HOST_DEVICE static constexpr auto pp_TVbMbKL_to_offset(Shape_MKL shape_mkl) {
    constexpr auto block_layout = ppblock_TV_to_offset();

    // (BlocksM, BlocksK, L)
    auto blocks_shape = cute::transform(shape_mkl, append(PPBlockShape_MK{}, _1{}),
                                        [](auto x, auto y) { return x / y; });

    // ((athrid, val), (BlocksM, BlocksK, L)) -> (storage_idx)
    auto result =
        make_layout(block_layout,
                    make_layout(blocks_shape, compact_col_major(blocks_shape, size(block_layout))));

    // ((athrid, val), (BlocksM, BlocksK, L)) => ((athrid, val), (BlocksM, BlocksK), L)
    return group<1, 3>(result(_, repeat<rank<1>(result)>(_)));
  }

  // ((BlockM, BlockK), (BlocksM, BlocksK), L) -> (storage_idx)
  template <typename Shape_MKL>
  CUTE_HOST_DEVICE static constexpr auto pp_MKbMbKL_to_offset(Shape_MKL shape_mkl) {
  ...
  CUTE_HOST_DEVICE static auto pp_TVbMbK_to_MKL(Shape_MKL shape_mkl) {
    auto tile =
        make_tile(make_layout(size<0>(PPBlockShape_MK{})), make_layout(size<1>(PPBlockShape_MK{})));
  ...
  CUTE_HOST_DEVICE static auto pp_MKL_to_TVbMbK(Shape_MKL shape_mkl) {
    ...
    return blocked_product(ppblock_MK_to_TV(), make_layout(shape<1>(TVbMbK_to_MKL_layout)));
  }
};
