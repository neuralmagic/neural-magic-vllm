...
template <class ElementATuple_, class GmemLayoutA, int AlignmentA, class ElementB_,
          class GmemLayoutB, int AlignmentB, class ElementAccumulator_, class TileShape_MNK,
...
public:
  ...
  using PPBlocksPerTile_MK =
      decltype(make_shape(size<0>(TileShape_MNK{}) / size<0>(PPBlockShape_MK{}),
  ...
  using ElementATuple      = cute::conditional_t<!cute::is_tuple<ElementATuple_>::value,
                                            cute::tuple<ElementA>, ElementATuple_>;

  static constexpr cute::GMMA::Major GmmaMajorA = gmma_rs_tag_to_major_A<StandardGmemLayoutA>();
...
private:
  ...
  static_assert(IsANarrow,
                "A must be the narrow one since its the one that flows through registers.");
  static constexpr bool IsWarpSpecializedTransposeB =
      is_warpspecialized_transpose_B<ElementA, StandardGmemLayoutA, ElementB, GmemLayoutB,
                                     KernelScheduleType>();

public:
  static constexpr int PipelineStages =
      compute_stage_count_or_override_single_affine_transformed_input<
          sm90_smem_capacity_bytes, ElementA, ElementB, ElementScale, ElementZero, TileShape_MNK>(
          StageCountType{});
  ...
  using SmemLayoutAPrepacked = decltype(GmemLayoutA::pp_TVbMbKL_to_offset(make_shape(
      size<0>(TileShape_MNK{}), size<2>(TileShape_MNK{}), Int<DispatchPolicy::Stages>{})));
  ...
  static_assert((size<0>(TileShape{}) % size<0>(SmemLayoutAtomAStandard{})) == 0,
                "SmemLayoutAtom must evenly divide tile shape.");
  static_assert((size<2>(TileShape{}) % size<1>(SmemLayoutAtomAStandard{})) == 0,
                "SmemLayoutAtom must evenly divide tile shape.");

  using SmemLayoutAtomB =
      decltype(rs_smem_selector<GmmaMajorB, ElementB, decltype(cute::get<1>(TileShape_MNK{})),
                                decltype(cute::get<2>(TileShape_MNK{})),
                                IsWarpSpecializedTransposeB>());
...
public:
  ...
  static_assert((sizeof(ElementB) == 2) || (cutlass::gemm::detail::is_k_major<StrideA>() &&
                                            cutlass::gemm::detail::is_k_major<StrideB>()),
                "The unscaled element (matrix B) must be 2 bytes OR both inputs must be K-major");
  ...
  using SmemLayoutAtomScale =
      Layout<Shape<decltype(cute::shape<0>(SmemLayoutAtomAStandard{})), cute::Int<1>>>;
  ...
  // If A mn-layout and B mn-layout, transposing B matrix since WGMMA is k-major only (e.g. tf32,
  // fp32, fp8, int8).
  ...
  using TransposeOperandB =
      decltype(cutlass::transform::collective::detail::make_transpose_operand_b(
          0, 0, TiledMma{}, SmemLayoutB{}, SmemLayoutAtomB{}, InternalElementB{},
  ...
  using GmmaSmemLayoutAtomB =
      decltype(transform::collective::detail::gmma_smem_transpose_or_passthrough<
  ...
  static_assert(!TransposeB ||
                    size<1>(SmemLayoutB{}) * cute::sizeof_bits_v<InternalElementB> / 8 == 128,
...
public: // TODO: make private
  ...
  static constexpr uint32_t compute_tma_transaction_bytes() {
    constexpr uint32_t a_bytes = (size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) *
                                  static_cast<uint32_t>(cute::sizeof_bits_v<InternalElementA>) / 8);
    constexpr uint32_t b_bytes = (size<0>(SmemLayoutB{}) * size<1>(SmemLayoutB{}) *
  ...
  using AStandardTensor =
      decltype(make_tensor(get_logical_ptr(static_cast<InternalElementA const*>(nullptr)),
                           repeat_like(StrideA{}, int32_t(0)), StrideA{}));

  // ((athrid, val), TilesM, TilesK, L) -> (storage_idx)
  using PrepackedStrideA = decltype(stride(
      GmemLayoutA::pp_TVbMbKL_to_offset(make_shape(int32_t(0), int32_t(0), int32_t(0)))));

  using APrepackedTensor = decltype(make_tensor(
      get_logical_ptr(static_cast<InternalElementA const*>(nullptr)),
      shape(GmemLayoutA::pp_TVbMbKL_to_offset(make_shape(int32_t(0), int32_t(0), int32_t(0)))),
      PrepackedStrideA{}));

  using ATensor = std::conditional_t<ALayoutIsPrepacked, APrepackedTensor, AStandardTensor>;

  using ScaleTensor =
      decltype(make_tensor(get_logical_ptr(static_cast<NonVoidElementScale const*>(nullptr)),
                           repeat_like(NonVoidStrideScale{}, int32_t(0)), NonVoidStrideScale{}));

  using ZeroTensor =
      decltype(make_tensor(get_logical_ptr(static_cast<NonVoidElementZero const*>(nullptr)),
                           repeat_like(NonVoidStrideScale{}, int32_t(0)), NonVoidStrideScale{}));

  using BTensor =
      decltype(make_tensor(get_logical_ptr(static_cast<InternalElementB const*>(nullptr)),
  ...
  static constexpr auto make_tma_copy_A(ATensor tensor_a = ATensor{}) {
    if constexpr (ALayoutIsPrepacked) {
      return make_tma_copy<TmaElementA>(
          GmemTiledCopyA{}, tensor_a, SmemLayoutAPrepacked{}(_, _, cute::Int<0>{}),
  ...
  static constexpr auto make_tma_copy_zero(ZeroTensor tensor_zero = ZeroTensor{}) {
    return make_tma_copy(GmemTiledCopyScale{}, tensor_zero, SmemLayoutScale{}(_, _, cute::Int<0>{}),
...
public:
  ...
  static constexpr Params to_underlying_arguments(ProblemShape const& problem_shape,
    ...
    if constexpr (ALayoutIsPrepacked) {
      ...
      tma_load_a = make_tma_copy_A(make_logical_tensor(ptr_A, make_shape(M, K, L), args.dA));
    }
    ...
    if constexpr (ModeHasScales) {
      tma_load_scale = make_tma_copy_scale(
          make_logical_tensor(args.ptr_S, make_shape(M, args.group_size, L), args.dS));
    }

    if constexpr (KernelConversionMode == ConversionMode::ConvertAndScaleWithZero) {
      tma_load_zero = make_tma_copy_zero(
          make_logical_tensor(args.ptr_Z, make_shape(M, args.group_size, L), args.dS));
    }
  ...
  CUTLASS_HOST_DEVICE static bool can_implement(ProblemShape const&               problem_shape,
    ...
    if (!implementable) {
      CUTLASS_TRACE_HOST("  CAN IMPLEMENT: Problem Size doesn't meet the minimum alignment "
  ...
  CUTLASS_DEVICE auto load_init(ProblemShape_MNKL const& problem_shape_MNKL,
    ...
    auto make_gA_mkl = [&]() {
      if constexpr (ALayoutIsPrepacked) {
        ...
        Tensor mA_mkl    = mainloop_params.tma_load_a.get_tma_tensor(shape(pp_layout));
        return local_tile(mA_mkl, make_shape(size<0>(pp_layout), PPBlocksPerTile_MK{}),
                          make_coord(0, make_coord(_, _)));
      } else {
        Tensor mA_mkl = mainloop_params.tma_load_a.get_tma_tensor(make_shape(M, K, L));
        return local_tile(mA_mkl, TileShape{}, make_coord(_, _, _), Step<_1, X, _1>{});
      }
    };

    // (BLK_N,BLK_K,n,k,l)
    auto make_gB_nkl = [&]() {
      Tensor mB_nkl = mainloop_params.tma_load_b.get_tma_tensor(make_shape(N, K, L));
      return local_tile(mB_nkl, TileShape{}, make_coord(_, _, _), Step<X, _1, _1>{});
    };

    // (BLK_M,BLK_Scale_K,m,scale_k,l)
    auto make_gS_mkl = [&]() {
      auto   scale_k = mainloop_params.scale_k;
      Tensor mS_mkl  = mainloop_params.tma_load_scale.get_tma_tensor(make_shape(M, scale_k, L));
    ...
    auto make_gZ_mkl = [&]() {
      auto   scale_k = mainloop_params.scale_k;
      Tensor mZ_mkl  = mainloop_params.tma_load_zero.get_tma_tensor(make_shape(M, scale_k, L));
    ...
    if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
      ...
      return cute::make_tuple(make_gA_mkl(), make_gB_nkl(), make_gS_mkl(), make_gZ_mkl());
    } else {
      static_assert(cutlass::detail::dependent_false<KernelSchedule>,
  ...
  CUTLASS_DEVICE void mma(MainloopPipeline pipeline, PipelineState smem_pipe_read,
    ...
    Tensor sA  = as_position_independent_swizzle_tensor(sA_); // (T, MMA, MMA_M, MMA_K, pipe)
    ...
    Tensor gmma_sB =
        as_position_independent_swizzle_tensor(gmma_sB_position_dependent); // (BLK_N,BLK_K,PIPE)
    ...
    // static constexpr int K_BLOCKS_PER_COPY = size<0>(group<0, 2>(tCrA_load)) / A_CPY_VEC;
    static constexpr int COVERSION_WIDTH = std::min(A_CPY_VEC, int(size<0>(tCrA_mma)));

    auto load_A_to_registers = [&](int read_stage) {
      copy(AutoVectorizingCopyWithAssumedAlignment<A_CPY_VEC>{}, tCsA(_, _, _, read_stage),
           tCrA_load(_, _, _));
    };
    ...
    auto convert_A = [&, a_vec = Int<COVERSION_WIDTH>{}](int k_block, int read_stage) {
      load_extra_info_to_registers(partitioned_extra_info, copy_partitions_extra_info, k_block,
                                   read_stage);
    ...
    TransposeOperandB transpose = transform::collective::detail::make_transpose_operand_b(
        warp_idx, warp_group_thread_idx, tiled_mma, SmemLayoutB{}, SmemLayoutAtomB{},
...
private:
  ...
  template <class... Ts>
  CUTLASS_DEVICE auto
  partition_extra_tma_inputs(Params const& mainloop_params, cute::tuple<Ts...> const& load_inputs,
  ...
  CUTLASS_DEVICE auto partition_extra_mma_info(ThreadMma const& thread_mma,
    ...
    if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
      ...
      Tensor tCrS =
          make_tensor<ElementScale>(thread_mma.partition_fragment_A(sS(_, _, Int<0>{})).shape());
      ...
      if constexpr (KernelConversionMode == ConversionMode::ConvertAndScale) {
        ...
        Tensor tCrZ =
            make_tensor<ElementZero>(thread_mma.partition_fragment_A(sZ(_, _, Int<0>{})).shape());
  ...
  CUTLASS_DEVICE void
  load_extra_info_to_registers(cute::tuple<Ts...> const& partitioned_mma_extra_info,
  ...
  CUTLASS_DEVICE void
  transform_A_kblock(TCrA_load const& tCrA_load, cute::Int<VectorWidthA> vec_A, TCrA_mma& tCrA_mma,
  ...
  CUTLASS_DEVICE void convert_tensor(Tensor<EngineIn, TensorLayout> const& in,
    ...
    using Converter =
        cutlass::NumericArrayConverter<DstType, SrcType, ConversionVectorWidth, RoundStyle>;
