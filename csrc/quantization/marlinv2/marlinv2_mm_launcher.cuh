...
torch::Tensor run_impl(PytorchArguments args) {
  ...
  auto scales_ptr = maybe_data_ptr<ElementScale const, LayoutScale>(args.scales, "scales");
  auto zeros_ptr  = maybe_data_ptr<ElementZero const, LayoutZero>(args.zeros, "zeros");

  auto arguments = KernelSpeacialization::create_arguments(
      stream, M, N, K, A_ptr, B_ptr, C_ptr, D_ptr, scales_ptr, zeros_ptr, args.alpha.value_or(1),
      args.beta.value_or(0), args.group_size.value_or(K));
