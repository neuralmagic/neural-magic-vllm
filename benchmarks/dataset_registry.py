...
def get_ultrachat(tokenizer: PreTrainedTokenizerBase,
    ...
    for messages in ds["messages"]:
        for i in range(1, len(messages), 2):
            ...
            completions.append(messages[i]["content"])
...
RAG_PREFILL_LENS = [2048,2560,3072,3584]
RAG_DECODE_LEN = 128


def get_synthetic_rag(tokenizer: PreTrainedTokenizerBase,
                      dataset_args: DatasetArgs) -> DatasetTriple:
    
    def _make_txt_with_n_tokens(n=1024):
        prompt = "Hello_" * (n // 2)
        return prompt[:-1] 
    
    prompts = []
    for prefill_len in RAG_PREFILL_LENS:
        prompts.append(_make_txt_with_n_tokens(prefill_len))

    prompts = (prompts * dataset_args.num_samples)[:dataset_args.num_samples]
    completions = [_make_txt_with_n_tokens(RAG_DECODE_LEN)] * dataset_args.num_samples
...
_DATASET_REGISTRY = {
    ...
    "sonnet": get_sonnet,
    "synthetic_rag": get_synthetic_rag,
}
