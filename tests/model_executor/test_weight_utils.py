...
def test_hf_model_weights_iterator_dense_compressor(hf_folder_dense):
    ...
    loaded_weight_names = set(
        name for name, _ in hf_model_weights_iterator(hf_folder_dense))
    assert model_weight_names == loaded_weight_names


def test_hf_model_weights_iterator_sparse_compressor(tmp_path,
                                                     hf_folder_sparse):
    ...
    model_weight_names = set([name for name, _ in model.named_parameters()])
    loaded_weight_names = set(
        name for name, _ in hf_model_weights_iterator(save_path))
    assert model_weight_names == loaded_weight_names
